...
int main(){
    ...
    //Unlike sets, unordered multisets store elements that can have duplicate Keys.
    //Similar to unordered sets this is also implemented as hash table where the key is hashed and placed into "buckets" based on the hash value.
    //Keys with similar hash values are stored into same "buckets". Therefore, the avg time complexities is constant. 

    s.insert("Banana");
    s.insert("Papaya");
    s.insert("Papaya"); //inserting duplicate keys
    s.insert("Papaya");
    s.insert("Mango");
    printData(s);

    std::cout<< "Number of elements present in the multiset: " << s.size() << std::endl; //returns the number of elements present in the set
    std::cout<< "Size of unordered_multiset object: "<< sizeof(s) <<std::endl; //returns the size of the actual unordered_set object
    ...
    s.erase("Papaya"); //erase using Key //erasing using duplicate keys will erase every instance of it
    ...
    std::unordered_multiset<vertex, hashVertex> s2;
    s2.insert(vertex(7, 10));
    s2.insert(vertex(7, 10)); //inserting duplicate keys
